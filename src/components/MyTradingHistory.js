...
import MyTradingHistoryItem from './MyTradingHistoryItem';
...
export default function MyTradingHistory({ stocks, paddingBottom, bottomText }) {
  ...
  <TouchableOpacity key = {element} onPress={() => navigation.navigate('StockPage', {stockName: element})}>
    <MyTradingHistoryItem key = {element} stockName={element} userShares={DummyStocks[element].userShares} shareValue={DummyStocks[element].shareValue} previousValue={DummyStocks[element].previousValue} />
  </TouchableOpacity>
  ));
